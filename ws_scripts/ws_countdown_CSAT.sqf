...
[["ws_alert",["We have begun securing the HEMTT!"]],"bis_fnc_showNotification",opfor] spawn BIS_fnc_MP;
[["ws_alert",["CSAT have begun seizing the cargo!"]],"bis_fnc_showNotification",blufor] spawn BIS_fnc_MP;

while {true} do {

	while {{(side _x == opfor && _x distance c3 <= 10)} count playableUnits > 0} do {
		sleep 5;
		ws_time_passed_o = ws_time_passed_o + 5;
			...
			if (ws_time_passed_o == (_time - 10)) then {
				[["ws_alert",["10 seconds to secure the cargo!"]],"bis_fnc_showNotification",opfor] spawn BIS_fnc_MP;
			};
	...
	if (_done && !ws_caches_seized_n) exitWith {
		...
		[["ws_alert",["We have secured the HEMTT!"]],"bis_fnc_showNotification",opfor] spawn BIS_fnc_MP;
		[["ws_alert",["CSAT has secured the HEMTT!"]],"bis_fnc_showNotification",blufor] spawn BIS_fnc_MP;
		ws_caches_seized_o = true;
		publicVariable "ws_caches_seized_o";
	};

	[["ws_alert",["We need to stay next to the truck!"]],"bis_fnc_showNotification",opfor] spawn BIS_fnc_MP;
	sleep 5;
};