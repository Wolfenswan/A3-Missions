...
for [{_i = 0}, {_i < 100 && !_exit}, {_i = _i + 1}] do {
  ...
  //_p = _pos findEmptyPosition [0,_dst,_veh];
	_p = (_pos) isFlatEmpty [ (sizeof _veh), 0, 0.7, (sizeof _veh),  0, false, objNull ];
	//diag_log str [ (sizeof _veh), 40, 0.7, (sizeof _veh),  0, false, objNull ];
	...
	//if (_i > 5) then { sleep 0.001; };
};
if (_i > 99) then {
_pos = [];
};
//diag_log format ["attempts: %1",_i];
